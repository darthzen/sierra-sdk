...
static char* readline(int fd)
{
  ...
  for (i = 0, c = 0; c != '\n'; i++)
  {
    if (i == input_bufsize)
    {
      char* temp_input_buf = NULL;
      input_bufsize += 128;
      temp_input_buf = realloc(input_buf, input_bufsize);
      if (NULL == temp_input_buf)
      {
        SWI_LOG("APPMON", ERROR, "readline: realloc error\n");
        return NULL;
      }
      input_buf = temp_input_buf;
    }
    if (1 != read(fd, &c, 1))
    {
      //free(buf);
...
static char* fill_output_buf(char* fmt, ...)
{
  ...
  {
    char* temp_output_buf = NULL;
    output_bufsize = towrite + 128;
    temp_output_buf = realloc(output_buf, output_bufsize);
    if (NULL == temp_output_buf)
    {
      SWI_LOG("APPMON", ERROR, "fill_output_buf: realloc error\n");
      return NULL;
    }
    output_buf = temp_output_buf;
    va_start(argp, fmt);
