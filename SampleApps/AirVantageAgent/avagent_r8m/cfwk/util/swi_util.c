...
swi_status_t swi_util_Get(const char* pathPtr, swi_dset_Iterator_t** data)
{
...
swi_status_t swi_util_MultipleGet(size_t numVars, const char** pathPtr, swi_dset_Iterator_t** data)
{
  int i;
  swi_status_t res;
  ...
  for (i = 0; i < numVars; i++)
  {
    res = swi_util_Get(pathPtr[i], &tmp);
...
swi_status_t swi_util_SetInteger(const char* pathPtr, int value)
{
  return setVariable(pathPtr, &value, DT_TYPE_INTEGER);
}

swi_status_t swi_util_SetFloat(const char* pathPtr, double value)
{
  return setVariable(pathPtr, &value, DT_TYPE_DOUBLE);
}

swi_status_t swi_util_SetString(const char* pathPtr, const char* valuePtr)
{
  return setVariable(pathPtr, (void *)valuePtr, DT_TYPE_STRING);
}

swi_status_t swi_util_SetNull(const char *pathPtr)
{
  return setVariable(pathPtr, NULL, DT_TYPE_NULL);
}

swi_status_t swi_util_SetBool(const char* pathPtr, bool value)
{
  return setVariable(pathPtr, &value, DT_TYPE_BOOL);
}

swi_status_t swi_util_Register(size_t numRegVars, const char** regVarsPtr, swi_util_NotifyCB_t cb,
    size_t numPassiveVars, const char** passiveVarsPtr, swi_util_regId_t* regIdPtr)
{
  swi_status_t res;
...
swi_status_t swi_util_Unregister(swi_util_regId_t regId)
{
  swi_status_t res;
  char* payload, *respPayload = NULL;
  size_t payloadLen;
