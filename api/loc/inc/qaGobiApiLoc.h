 ...
 *
 *  \param  pLOCEventRegisterReqResp
 ...
 *
 * \param pHorizontalAccuracyLvl[IN]
 *        - Optional Parameter
 ...
 *           - 3 - High accuracy
 *
 * \param pIntermediateReportState[IN]
 *        - Optional Parameter
 ...
 *
 *  \param  request[IN]
 *          - request structure parameters should contain all NULL pointers to
 *            delete all assistance data. Otherwise, specify optional fields
 *            to be deleted.
 *            See \ref LocDelAssDataReq for more information
 *
 *  \return eQCWWAN_ERR_NONE on success, eQCWWAN_xxx error value otherwise
 *
 *  \sa     See qmerrno.h for eQCWWAN_xxx error values
...
/**
 *  Control point to get the best available position estimate from the location engine.
 *
 *  \param  xid[IN]
 ...
 *
 *  \note   Timeout: 5 seconds
 *          - GPS engine should be started to get best available position.
 *
 */
ULONG SLQSLOCGetBestAvailPos(ULONG xid);
...
/**
 *  Gets the A-GPS server address from the engine.
 *  The request is acknowledged through the response, but the SUCCESS/FAILURE
 *  status is sent through \ref SetLocGetServerCallback callback
 *  If successful, the callback also contains the A-GPS server address
 *
 *  \param  pLocGetServerReq[IN]
 ...
 *
 */
ULONG SLQSLOCGetServer(LocGetServerReq *pLocGetServerReq);
